...
export class ReminderService {
  ...
  getReminders(resourceParams: ReminderResourceParameter): Observable<HttpResponse<Reminder[]>> {
  ...
  addReminder(reminder: Reminder): Observable<Reminder | CommonError> {
    const url = `reminder`
    return this.httpClient.post<Reminder>(url, reminder).pipe(catchError(this.commonHttpErrorService.handleError))
  }
  ...
  deleteReminder(id: string): Observable<Reminder | CommonError> {
    ...
    return this.httpClient.delete<Reminder>(url).pipe(catchError(this.commonHttpErrorService.handleError))
  }
}
