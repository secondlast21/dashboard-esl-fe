...
export class DocumentService {
  ...
  updateDocument(document: DocumentInfo): Observable<DocumentInfo | CommonError> {
    ...
    return this.httpClient.put<DocumentInfo>(url, document).pipe(catchError(this.commonHttpErrorService.handleError))
  }

  addDocument(document: DocumentInfo): Observable<DocumentInfo | CommonError> {
    const url = `document`
    return this.httpClient.post<DocumentInfo>(url, document).pipe(catchError(this.commonHttpErrorService.handleError))
  }
  ...
  getDocument(id: string): Observable<DocumentInfo | CommonError> {
    const url = `document/${id}`
    return this.httpClient.get<DocumentInfo>(url).pipe(catchError(this.commonHttpErrorService.handleError))
  }
  ...
  getDocumentVersion(id: string) {
    ...
    return this.httpClient.get<DocumentVersion[]>(url).pipe(catchError(this.commonHttpErrorService.handleError))
  }

  restoreDocumentVersion(id: string, versionId: string) {
    const url = `documentversion/${id}/restore/${versionId}`
    return this.httpClient.post<boolean>(url, {}).pipe(catchError(this.commonHttpErrorService.handleError))
  }
